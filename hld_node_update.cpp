...
struct LazySegTree {
    ...
    void push(int node, int l, int r) {
        if (lazy[node] != 0) {
            tree[node] += lazy[node] * (r - l + 1); // Range Sum
            if (l != r) {
    ...
    ll query(int node, int l, int r, int ql, int qr) {
        push(node, l, r);
        if (ql > r || qr < l) return 0; // 0 for Sum
...
struct HLD {
    ...
    HLD(int n) : n(n), timer(0), adj(n), parent(n), depth(n), head(n), 
                 pos(n), sz(n), heavy(n, -1), seg(n) {}
    ...
    // Call this FIRST to set up the chains
    void build_tree(int root = 0) {
        dfs_sz(root);
        dfs_hld(root, -1, root);
    }

    // Call this SECOND if you have initial values on the nodes
    void init_values(const vector<ll>& initial_vals) {
        vector<ll> flat_vals(n);
        for (int i = 0; i < n; i++) {
            flat_vals[pos[i]] = initial_vals[i];
        }
    ...
    // --- Point Operations ---
    void update_point(int u, ll val) {
        seg.update(1, 0, n - 1, pos[u], pos[u], val);
    }

    ll query_point(int u) {
        return seg.query(1, 0, n - 1, pos[u], pos[u]);
    }
    
    // --- Subtree Operations ---
    void update_subtree(int u, ll val) {
        seg.update(1, 0, n - 1, pos[u], pos[u] + sz[u] - 1, val);
    }
    
    ll query_subtree(int u) {
        return seg.query(1, 0, n - 1, pos[u], pos[u] + sz[u] - 1);
    }
};
